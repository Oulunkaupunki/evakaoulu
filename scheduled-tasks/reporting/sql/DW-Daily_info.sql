SELECT
    ...
    array(
        ...
        FROM absence
        WHERE child_id = p.id
            AND a.date = (current_date AT TIME ZONE 'Europe/Helsinki')::date
    )                                       AS poissaolon_syy
FROM person p
    JOIN placement pl ON pl.child_id = p.id
        AND pl.start_date <= (current_date AT TIME ZONE 'Europe/Helsinki')::date
        AND pl.end_date >= (current_date AT TIME ZONE 'Europe/Helsinki')::date
    ...
    JOIN daycare_group_placement dgp ON pl.id = dgp.daycare_placement_id
        ...
        AND dgp.end_date >= (current_date AT TIME ZONE 'Europe/Helsinki')::date
    JOIN daycare_group dg ON d.id = dg.daycare_id
        AND dgp.daycare_group_id = dg.id
    LEFT JOIN backup_care bc ON p.id = bc.child_id
        AND bc.start_date <= (current_date AT TIME ZONE 'Europe/Helsinki')::date
        AND bc.end_date >= (current_date AT TIME ZONE 'Europe/Helsinki')::date
    ...
    LEFT JOIN service_need sn ON pl.id = sn.placement_id
        AND sn.start_date <= (current_date AT TIME ZONE 'Europe/Helsinki')::date
        AND sn.end_date >= (current_date AT TIME ZONE 'Europe/Helsinki')::date
    JOIN service_need_option sno ON sno.id = sn.option_id
    LEFT JOIN assistance_need an ON p.id = an.child_id
        AND an.start_date <= (current_date AT TIME ZONE 'Europe/Helsinki')::date
        AND an.end_date >= (current_date AT TIME ZONE 'Europe/Helsinki')::date
    LEFT JOIN assistance_need_voucher_coefficient anvc ON p.id = anvc.child_id
        AND lower(anvc.validity_period) <= (current_date AT TIME ZONE 'Europe/Helsinki')::date
        AND upper(anvc.validity_period) >= (current_date AT TIME ZONE 'Europe/Helsinki')::date
