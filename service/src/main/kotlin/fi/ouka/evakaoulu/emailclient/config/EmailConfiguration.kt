...
internal class EmailMessageProvider(
    ...
    override fun messageNotification(
        ...
        return EmailContent.fromHtml(
            ...
            html =
                ...
                $unsubscribeFi
                <hr>
                <p>You have received a new $typeEn in eVaka in eVaka from ${thread.senderName}${if (showSubjectInBody) " with title \"" + thread.title + "\"" else ""}. Read the message ${if (thread.urgent) "as soon as possible " else ""}in eVaka. </p>
    ...
    override fun childDocumentNotification(
        ...
        if (notificationType == ChildDocumentNotificationType.EDITABLE_DOCUMENT) {
            return EmailContent.fromHtml(
                subject =
                    "Uusi täytettävä asiakirja eVakassa / New fillable document in eVaka",
                html =
                    """
